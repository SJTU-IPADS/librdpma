...
**/
int nrfsWrite(nrfs fs, nrfsFile _file, const void* buffer, uint64_t size, uint64_t offset,uint64_t off1)
{
	...
	if(bufferExtentWriteReceive.result == true) {
		...
		for(int i = 0; i < (int)fpi.len; i++)
		{
			...
			{
#if !USE_R2
              u64 write_sz = fpi.tuple[i].size;
                          //ASSERT(write_sz < size) << "write sz: " << write_sz << " ; size: " << size;
                          //if (write_sz >= size) {
                            //write_sz = size;
                            //}
                          ...
                          write_sz = size;
                          u64 write_off =
                              fpi.tuple[i].offset + DmfsDataOffset + off1;
                          total_sz += write_sz;

                          if (write_off >= 4L * 1024 * 1024 * 1024) {
                            write_off = write_off % 4L * 1024 * 1024 * 1024L;
                          }
                          ...
                          auto res_s = nvm_qp->send_normal(
                              {.op = IBV_WR_RDMA_WRITE,
                               ...
                               .wr_id = 0},
                              {.local_addr =
                                   reinterpret_cast<RMem::raw_ptr_t>(local_ptr),
                               .remote_addr =
                               write_off,
                               .imm_data = 0});
                          ...
                          RDMA_ASSERT(res_p == IOCode::Ok)
                            << "write sz: " << fpi.tuple[i].size << " off: " << write_off
                            << "; err: " << RC::wc_status(res_p.desc);
#if USE_PERSIST
                          // add read
                          // emulate
                          res_s = nvm_qp->send_normal(
                              {.op = IBV_WR_RDMA_READ,
                               .flags = IBV_SEND_SIGNALED,
                               .len = sizeof(u8),
                               .wr_id = 0},
                              {.local_addr =
                               ...
                               .imm_data = 0});
                          RDMA_ASSERT(res_s == IOCode::Ok);
                          res_p = nvm_qp->wait_one_comp();
                          RDMA_ASSERT(res_p == IOCode::Ok);
#endif
#else
                          write_off += nvm_qp->remote_mr.value().buf;
                          // optimized persistent write
                          ...
                          doorbell.cur_wr().wr.rdma.remote_addr =
                            write_off;
                          doorbell.cur_wr().wr.rdma.rkey = nvm_qp->remote_mr.value().key;

                          doorbell.cur_sge() = {
                            .addr = (u64)(local_ptr),
                            .length = write_sz,
                            .lkey = nvm_qp->local_mr.value().key
                          };
                          ...
                          read_sr.wr.rdma.rkey = nvm_qp->remote_mr.value().key;

                          read_sge.addr = (u64)(local_ptr);
                          read_sge.length = sizeof(u8);
                          read_sge.lkey = nvm_qp->local_mr.value().key;

                          doorbell.freeze();
                          struct ibv_send_wr *bad_sr;
                          auto rc = ibv_post_send(nvm_qp->qp,doorbell.first_wr_ptr(),&bad_sr);
                          RDMA_ASSERT(rc == 0);

                          auto res_p = nvm_qp->wait_one_comp();
                          RDMA_ASSERT(res_p == IOCode::Ok) << RC::wc_status(res_p.desc);
#endif
#endif
                          break;
                        }
        ...
        //RDMA_ASSERT(length_copied >= size) << length_copied;
		gettimeofday(&end1, NULL);
		diff = 1000000 * (end1.tv_sec - start1.tv_sec) + end1.tv_usec - start1.tv_usec;
		WriteTime3 += diff;
