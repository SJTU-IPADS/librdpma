...
public:
  ...
  inline void post_reqs_impl(oltp::RScheduler *s, Qp *qp) {
  ...
  // use qp1 to send the req, use dram buf in qp for flush
  inline void post_reqs_impl_1(oltp::RScheduler *s, Qp *qp,Qp *qp1) {

    sr[0].wr.rdma.remote_addr += qp1->remote_attr_.memory_attr_.buf;
    sr[0].wr.rdma.rkey = qp1->remote_attr_.memory_attr_.rkey;
    sge[0].lkey = qp1->dev_->conn_buf_mr->lkey;

    sr[1].wr.rdma.remote_addr += qp->remote_attr_.memory_attr_.buf;
    sr[1].wr.rdma.rkey = qp->remote_attr_.memory_attr_.rkey;
    sge[1].lkey = qp->dev_->conn_buf_mr->lkey;

    s->post_batch(qp1, cor_id, &(sr[0]), &bad_sr, 1);
  }
};
