...
::rdmaio::Arc<MemoryRegion> nvm_region = nullptr;

int tcp_port = 33333;

::rdmaio::RCtrl ctrl(6666);
    ...
    : barrier_a_(1),barrier_b_(1),init_worker_count_(0),store_(NULL)
{
  if (current_partition == 0) {
    // ASSERT(false) << "not partiton 0";
    // we only have one machine w NVM
#if NUMA_OPT
    const std::string nvm_file_name = "/dev/dax0.1";
#else
    const std::string nvm_file_name = "/dev/dax1.3";
#endif
    nvm_region = (NVMRegion::create(nvm_file_name, 10L * (1024 * 1024 * 1024L)).value());
    LOG(4) << "nvm use region: " << nvm_file_name;
    ASSERT(nvm_region->valid());

    //nvm_region = HugeRegion::create(4 * 1024 * 1024 * 1024L).value();
    ctrl.start_daemon();
  }
  running = true;
...
BenchRunner::run() {
  ...
  LOG(4) << "use rbuf sz : " << ROCC_RBUF_SIZE_M  << " mb";

  int r_port = tcp_port;

  //
  if (nvm_region && USE_NVM) {
    rdma_buffer = (char *)nvm_region->addr;
    ASSERT(r_buffer_size <= nvm_region->sz) << "r_buffer_sz: " << r_buffer_size << " " << nvm_region->sz;
    ASSERT(r_buffer_size % (2 * 1024 * 1024) == 0);
  } else {
    rdma_buffer =
        (char *)malloc_huge_pages(r_buffer_size, HUGE_PAGE_SZ, HUGE_PAGE);
  }
  assert(rdma_buffer != NULL);

  // start creating RDMA
  cm = new RdmaCtrl(current_partition,net_def_,r_port,false);
  LOG(4) << "rdma: buffer !!!!: " << (void *)rdma_buffer;
