 ...
 public:
  ...
  void init_new_logger(MemDB **backup_stores) {
    ...
    if (nvm_region != nullptr) {
      LOG(4) << "use NVM region!";
      new_logger_ = new rtx::RpcLogger(
          rpc_, RTX_LOG_RPC_ID, RTX_LOG_CLEAN_ID, M2, MAX_BACKUP_NUM,
          (char *)(nvm_region->addr), total_partition, nthreads,
          (coroutine_num + 1) * RTX_LOG_ENTRY_SIZE);
    } else { // origion DRAM's case
      if (current_partition == 1) {
        ASSERT(false);
      }
      new_logger_ = new rtx::RpcLogger(
          rpc_, RTX_LOG_RPC_ID, RTX_LOG_CLEAN_ID, M2, MAX_BACKUP_NUM,
          (char *)(cm_->conn_buf_) + HUGE_PAGE_SZ, total_partition, nthreads,
          (coroutine_num + 1) * RTX_LOG_ENTRY_SIZE);
    }
